 ...
 * or photo. They call sj_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional Sjöstad Service & Energi AB.
 *
 ...
 * @return string
 */
function sj_sample_note() {
	return sj_lead( 'ar_riktig' ) ? '' : ' <span class="sj-sample">(platshållare)</span>';
}
 ...
 * @return string File name for sj_lead_img().
 */
function sj_lead_bild( $i ) {
	$bilder = (array) sj_lead( 'bilder', array() );
 ...
 * @return string
 */
function sj_ring_label() {
	$namn = sj_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, sj_lead( 'telefon' ) ) : 'Ring ' . sj_lead( 'telefon' );
}
