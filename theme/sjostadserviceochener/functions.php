<?php
/**
 * Sjöstad Service & Energi AB theme functions.
 *
 * Kept short on purpose. Design lives in theme.json, markup in templates and patterns.
 * Lead capture lives in the "Sjöstad Service & Energi AB Leads" plugin, so leads survive a theme change.
 *
 ...
 * Patterns read these through sj_lead() so one demo can be rebuilt for any firm.
 */
...
add_action(
	'wp_enqueue_scripts',
	function () {
		wp_enqueue_style( 'sjostadserviceochener', get_stylesheet_uri(), array(), wp_get_theme()->get( 'Version' ) );
	}
);
add_action(
	'after_setup_theme',
...
add_action(
	'init',
	function () {
		register_block_style( 'core/paragraph', array( 'name' => 'eyebrow', 'label' => __( 'Eyebrow label', 'sjostadserviceochener' ) ) );
		register_block_style( 'core/list', array( 'name' => 'checklist', 'label' => __( 'Tick list', 'sjostadserviceochener' ) ) );

		register_block_pattern_category( 'sjostadserviceochener-sections', array( 'label' => __( 'Sjöstad Service & Energi AB sections', 'sjostadserviceochener' ) ) );
		register_block_pattern_category( 'sjostadserviceochener-pages', array( 'label' => __( 'Sjöstad Service & Energi AB page layouts', 'sjostadserviceochener' ) ) );
	}
);
