...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'sjostadserviceochener-leads' ),
					'singular_name' => __( 'Lead', 'sjostadserviceochener-leads' ),
					'edit_item'     => __( 'Förfrågan', 'sjostadserviceochener-leads' ),
					'search_items'  => __( 'Search leads', 'sjostadserviceochener-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'sjostadserviceochener-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'sjostadserviceochener-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'sjostadserviceochener-leads' ),
			'npl_phone'    => __( 'Phone', 'sjostadserviceochener-leads' ),
			'npl_service'  => __( 'Needs', 'sjostadserviceochener-leads' ),
			'npl_timeline' => __( 'When', 'sjostadserviceochener-leads' ),
			'npl_zip'      => __( 'ZIP', 'sjostadserviceochener-leads' ),
			'npl_source'   => __( 'Source', 'sjostadserviceochener-leads' ),
			'npl_status'   => __( 'Status', 'sjostadserviceochener-leads' ),
			'date'         => __( 'Received', 'sjostadserviceochener-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'sjostadserviceochener-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'sjostadserviceochener-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'sjostadserviceochener-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'sjostadserviceochener-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'sjostadserviceochener-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'sjostadserviceochener-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'sjostadserviceochener-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'sjostadserviceochener-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'sjostadserviceochener-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'sjostadserviceochener-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'sjostadserviceochener-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'sjostadserviceochener-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
