...
add_filter(
	...
	function ( $exporters ) {
		$exporters['sjostadserviceochener-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'sjostadserviceochener-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'sjostadserviceochener-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'sjostadserviceochener-leads',
						'group_label' => __( 'Quote requests', 'sjostadserviceochener-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['sjostadserviceochener-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'sjostadserviceochener-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
